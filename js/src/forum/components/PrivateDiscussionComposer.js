...
export default class PrivateDiscussionComposer extends DiscussionComposer {
    init() {
        ...
        if (
            app.forum.attribute("byobuTag") &&
            app.forum.attribute("byobuTag").length > 0
        ) {
            extend(PrivateDiscussionComposer.prototype, "headerItems", function(
                items
            ) {
                items.remove("tags");
            });
        ...
        if (recipients.length < 2) {
            ...
            if (tag) {
                if (tag.data.attributes.isChild) {
                    ...
                    const parentTag = app.store.getBy(
                        "tags",
                        "id",
                        parentTagId
                    );
                ...
                console.error(
                    "fof/byobu: Could not find tag with slug " +
                        app.forum.attribute("byobuTag")
                );
            }
